...
public class Recommendation {
    ...
    @JsonIgnoreProperties({"hibernateLazyInitializer", "handler", "listings", "tenant"})
    private MasterProduct masterProduct;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "marketplace_product_id")
    @JsonIgnoreProperties({"hibernateLazyInitializer", "handler", "masterProduct", "tenant"})
    ...
    public enum RecommendationType {
        PRICE_MATCH, TEMPORARY_DISCOUNT, STOCK_CLEARANCE,
        MARGIN_PROTECTION, BUNDLE_OFFER, PRICE_INCREASE
    }
